...
- Cartão aprovado: `status: success` na hora com `amountCents` / `creditCents` / `recurringAmountCents` / `savedCards` (webhook pode repetir; é idempotente)
...
Ver [endpoints](../api/endpoints.md), [ADR-020](../decisions/ADR-020-cartao-no-painel.md) e [ADR-028](../decisions/ADR-028-assinatura-recorrente-planos.md).
