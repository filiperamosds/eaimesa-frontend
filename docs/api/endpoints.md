...
| GET | `/v1/platform/logs/{name}` | Platform | Entries paginadas; query `page` (default 1 = mais recente), `perPage` (1–200, default 50), `level`, `q` |
| POST | `/v1/platform/logs/{name}/clear` | Platform | Rotaciona para `{stem}2.log` (ex. `laravel2.log`) e cria o ativo vazio |
...
`GET /v1/platform/logs/{name}`: só basename `*.log` sob `storage/logs` (sem path traversal). Resposta: `page`, `perPage`, `total`, `lastPage`, `content` (texto da página), `entries[]` (`timestamp`, `env`, `level`, `message`, `raw`), `truncated`. `POST .../clear` move para `{stem}2.log` (backup anterior, se existir, para `{stem}2-YmdHis.log`). Front: `/admin/logs`. Ver [fatia 13](../product/fatia-13-log-viewer.md).
