...
const ALL_LINKS = [
  ...
  { href: "/painel/pagamento", label: "Pagamento", icon: "R$", service: false },
] as const;
...
export function PainelShell({ children }: { children: React.ReactNode }) {
  ...
  const onPagamento = path.startsWith("/painel/pagamento");
  ...
  const links = ALL_LINKS.filter((l) => {
    ...
    if (l.href === "/painel/pagamento" && !promptPayment && !onPagamento) return false;
    return true;
  });
    ...
    <div className="min-h-screen pb-24 sm:pb-0">
      <header className="sticky top-0 z-30 border-b border-line/80 bg-card/85 backdrop-blur-xl">
        ...
        <div className="mx-auto hidden max-w-[88rem] px-5 pb-3 sm:block">
          <PainelNav path={path} links={links} highlightPayment={promptPayment} />
        </div>
      </header>
      {prompt && !onPagamento ? (
      ...
      >
        <ul
          className={`grid px-2 py-2 ${
            links.length >= 6 ? "grid-cols-3" : links.length > 2 ? "grid-cols-5" : "grid-cols-2"
          }`}
        >
          {links.map((l) => {
            const active = path.startsWith(l.href);
            const pay = l.href === "/painel/pagamento" && promptPayment;
            return (
              <li key={l.href}>
                <Link
                  href={l.href}
                  className={`flex flex-col items-center gap-0.5 rounded-xl py-1.5 text-[11px] ${
                    active || pay ? "text-chili" : "text-ink-soft"
                  }`}
                >
...
function PainelNav({
  ...
  return (
    <nav aria-label="Painel" className="flex rounded-2xl bg-paper-2/80 p-1">
      {links.map((l) => {
        const active = path.startsWith(l.href);
        const pay = highlightPayment && l.href === "/painel/pagamento";
        return (
          <Link
            key={l.href}
            href={l.href}
            className={
              active
                ...
                : pay
                  ? "flex-1 rounded-xl px-3 py-2.5 text-center text-sm font-medium text-chili hover:text-chili-dark"
