...
- Após login: `owner` → painel; `staff` (garçom/caixa) → `/garcom`; `member.role=panel` → `/painel/pedidos`
...
- Painel bloqueia role `staff` **exceto** `member.role=panel` (só `/painel/pedidos`); garçom usa `/login?next=/garcom`
