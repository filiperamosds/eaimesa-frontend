...
- Perfil **Painel**: opção **Imprimir via grupos** no cadastro da equipe (`printViaGroups`). Sem a opção, a estação continua com uma via só dos itens dela
...
4. Tablet da Cozinha (Painel) sem o flag: uma via só de comida. Com **Imprimir via grupos**: aplica os grupos do estabelecimento sobre os itens visíveis.
