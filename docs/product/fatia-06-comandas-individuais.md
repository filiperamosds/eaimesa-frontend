...
- Garçom `/garcom`: toque na mesa (livre ou ocupada) abre o **dialog**. Vê o **PIN**, abre comanda (nome + telefone), parcial com **A receber**, **Imprimir** (cupom 80&nbsp;mm isolado, sem o HTML do app), lança pedido, fecha conta.
...
4. Seleciona uma conta → **A receber** em destaque, itens e **Imprimir**. O cupom vai para a térmica como documento 80&nbsp;mm só com o texto da comanda (não o CSS/JS da página). Na caixa do sistema: papel 80&nbsp;mm, sem cabeçalho/rodapé; não usar A4. **Adicionar pedido**: escolhe categorias (só aí há **Cancelar**), soma itens de várias categorias (ex. bebida + petisco) e **Lança** um pedido.
