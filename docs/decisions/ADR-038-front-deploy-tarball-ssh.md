...
3. No servidor, `rsync --delete` (se existir) alinha a pasta com o tarball e limpa hashes velhos em `_next/`. Preserva `cgi-bin`, `.well-known`, `.env`, a pasta **`dev/`** (staging ao lado da produção no mesmo `public_html`) e pastas da API (`api`, `artisan`, `vendor`, `app`…). Recusa extrair em `$HOME`, `domains/` ou numa pasta Laravel — o FTP antigo não apagava extras; um `--delete` no sítio errado tira o `public` da API e o login no front passa a falhar com CORS (a API responde 404 HTML sem `Access-Control-Allow-Origin`).
