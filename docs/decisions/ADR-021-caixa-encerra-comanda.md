...
- Flag do estabelecimento `staffCanCloseTabs` (default **true**, para não quebrar o salão atual). Desligada: garçom toma 403 `CASHIER_REQUIRED`; a UI esconde os botões.
- Dono ajusta a flag em **Configurações → Estabelecimento** (`PATCH /v1/owner/venue`). Cadastra caixa em **Configurações → Equipe** (`role` no POST/PATCH `/v1/owner/staff`).
...
| Default `staffCanCloseTabs=false` | Estabelecimentos sem caixa cadastrado deixam de fechar mesa |
