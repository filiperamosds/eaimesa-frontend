...
| Estoque / receita | Só dono + módulo `inventory`; `venue_id` da sessão; pedido não é recusado por saldo |
...
- O extract faz `rsync --delete` só na pasta do front (`REMOTE_TARGET_FRONT_*` / `FTP_SERVER_DIR_*`), não no home SSH inteiro. Excludes Laravel são `/app/`, `/vendor/`, não `app` solto ([ADR-040](../decisions/ADR-040-rsync-exclude-chunks-app.md)). Staging em `public_html/dev` entra no exclude `/dev/` para o deploy da `main` não apagar.
