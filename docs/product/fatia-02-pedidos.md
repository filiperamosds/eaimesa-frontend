...
- **Imprimir** (ESC/POS USB). Auto-print liga no card em Configurações → Estabelecimento (salva no venue); **Configurar impressora** autoriza a POS80 neste Chrome (não depende do Salvar). Só imprime sozinho com a flag ligada; desligada, o pedido não fica na fila. Com **grupos de impressão**, cada grupo sai numa via com corte ([fatia 19](fatia-19-grupos-impressao.md)).
