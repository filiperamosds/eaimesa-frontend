...
- Painel `/painel/pagamento`: escolhe **cartão ou PIX**. Cartão: o POST leva `{ plan, method, creditCard }` (e `payer` só se alterado). Banner só quando o trial está acabando ou `past_due`
