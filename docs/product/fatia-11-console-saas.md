...
Login da **plataforma**, não do dono do estabelecimento. O operador vê vendas da assinatura, a lista de estabelecimentos e o catálogo de planos.
...
1. Operador entra em `/admin/login`. Se `GET /v1/platform/auth/me` já estiver ok, vai direto a `/admin`. Cookie `eaimesa_platform` é independente de `eaimesa_owner` — dá para operar o console e o painel do estabelecimento no mesmo browser.
2. Dashboard mostra os estabelecimentos do seed (trial → MRR 0) e os checkouts stub. Status e plano aparecem em português (Em trial, Ativo, Cardápio…).
